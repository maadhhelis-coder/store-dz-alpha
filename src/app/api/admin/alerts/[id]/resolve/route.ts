...
// حل تنبيه — integrations.manage (مالكوية). من open أو acknowledged مباشرة؛
// التنبيه المحلول مسبقًا يُرفض بـ409 حتميًا.
...
export async function POST(_request: Request, { params }: RouteParams) {
  try {
    ...
    await resolveAlert(parsed.data.id, admin.id);
